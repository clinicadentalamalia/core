...
export function PatientList() {
  ...
  return <><div className="toolbar"><div className="search"><Search size={18}/><input className="input" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nombre o identificador…" aria-label="Filtrar pacientes ficticios"/></div></div>
    ...
    <div className="patient-cards">{patients.map((patient) => <article className="card patient-card" key={patient.id}><div className="patient-cell"><div className="avatar">{patient.initials}</div><div>{patient.name}<div className="muted" style={{fontSize: ".72rem",fontWeight:500}}>{patient.identifier}</div></div><StatusBadge status={patient.status}/></div><p className="muted" style={{fontSize: ".8rem"}}>{patient.phone}<br/>{patient.nextAppointment}</p>{patient.alert && <div className="patient-alert"><AlertTriangle size={13}/>{patient.alert}</div>}</article>)}</div>
    {patients.length === 0 && <div className="placeholder"><div><h3>Sin coincidencias</h3><p className="muted">Prueba con otro nombre ficticio.</p></div></div>}
  </>;
}
